...
class RadioGroup extends Component {
  ...
  render () {
    ...
    const radioGroup = map(radioGroupList, (radiocomp, index) => {
      // var btn_style = buttonContainerInactiveStyle
      // var bg_color = result(radiocomp, 'color', '');
      // if(bg_color){
      //   btn_style.backgroundColor = bg_color;
      // }
      return <RadioButton key={index} label={result(radiocomp, 'label', '')}
        value={result(radiocomp, 'value', '')} bgColor={result(radiocomp, 'color', '')} index={index}
